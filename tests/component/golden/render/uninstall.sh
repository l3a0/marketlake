...
# 1. Boot the six jobs out. This undoes install step 5, and it leads so that
...
if launchctl print system/com.marketlake.eod-sweep >/dev/null 2>&1; then
  echo '+ sudo launchctl bootout system/com.marketlake.eod-sweep'
  sudo launchctl bootout system/com.marketlake.eod-sweep
else
  echo '  system/com.marketlake.eod-sweep is not loaded, nothing to boot out'
fi
...
# 4. Delete the six plists. This undoes install step 1, the first thing the
...
echo '+ sudo rm -f /Library/LaunchDaemons/com.marketlake.eod-sweep.plist'
sudo rm -f /Library/LaunchDaemons/com.marketlake.eod-sweep.plist
