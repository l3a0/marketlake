...
- **Unowned.** The install's first-ping verification. `capture` arms only when it has been pinged once, so an install whose first cycle fails leaves the whole-daemon guarantee inert while every job reports healthy. `pre-open` does not cover it, because it asserts the daemon is up rather than that capture works. The install text ends on `launchctl print`, which answers the wrong question. It should end on a read of the `capture` row, and treat `Never` as an incomplete install.
