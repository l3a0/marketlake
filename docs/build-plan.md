...
- **Unowned.** Tests for five of the daemon's production hook bindings. The hooks
  themselves are held, and so is each observer in isolation. What is unheld is the wiring
  `run_loop_from_config` builds, which is the wiring the launchd job actually runs. Five
  are in that state:

  1. the skipped-slot hook reaching `GapMarker`, so a live overrun would record no gap,
  2. the per-tick hook feeding the `capture` dead-man's idle heartbeat,
  3. the cycle hook feeding the same dead-man's `captured` signal, which is what arms the
     check on the first durable cycle and holds the whole-daemon guarantee,
  4. the per-tick hook reaching the close+5 guard's `SessionDispatch.check`,
  5. the per-cycle chain-plan re-read, which is what makes a nightly plan rewrite take
     effect the next minute.

  Each was confirmed by deleting the binding and running the suite, which stays green at
  1126. The point is not that the bindings are wrong. It is that nothing would notice if
  they became wrong, and three of the five are the paths that carry a failure to the
  phone. A sixth binding was on this list, the skipped-slot hook reaching the watchdog.
  `test_alert.py` holds it now, and deleting the binding fails that test.
