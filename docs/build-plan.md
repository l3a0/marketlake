...
- **Unowned.** The auth-gap reminder. The design sends a reminder while the token is dead, and the watchdog's cause page is once-on-transition, so a token that dies at 09:31 pages once and is then silent for the session. The page path exists now, so what remains is the schedule.
