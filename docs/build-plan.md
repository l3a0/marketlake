...
- **Unowned.** The `capture` dead-man ping and its idle heartbeats. The design feeds that check from every durable cycle plus heartbeats across the weekday envelope, and retires slice-1's `slice1-capture` check when it lands. Neither the feed nor the heartbeat is built. The operator step that goes with it is deleting the old healthchecks.io row, because a retired check left in place goes silent and pages for a job that no longer exists.
- **D14** laptop control plane. `render --out DIR` writes every plist and setup file to a directory and prints the install commands. It refuses a system directory, so installing stays the operator's by-hand step. Nothing here runs `sudo`, a `pmset` write, `launchctl bootstrap`, or a `tmutil` write. What does run is read-only and needs no root: `launchctl print` from the self-check, `pmset -g sched` and `tmutil isexcluded` from the Sunday job. The token path comes from one rule, so the daemon that rewrites it, the Sunday job that asserts coverage over it, and the exclusion that protects it cannot name different files. `RunAtLoad` is on for the two residents and the self-check. It is off for the Sunday job, which would otherwise scrub the whole lake at every boot. Beside the sudoers drop-in it renders four LaunchDaemons, and it prints the Time Machine exclusion as an install step rather than rendering it:
  ...
  4. the Sunday maintenance job, on its own.
