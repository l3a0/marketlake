...
def _decode(path: Path, raw: bytes) -> str:
    """A ledger's bytes as text, or a refusal naming what in them cannot be read.

    Two shapes refuse. Bytes that do not decode raise :class:`LedgerNotUtf8`. Text that decodes
    and holds a byte-order mark raises :class:`LedgerHasByteOrderMark`.
